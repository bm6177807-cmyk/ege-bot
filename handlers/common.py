...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Detect first launch before creating the user record
    ...
    if len(args) > 1:
        ...
        if param.startswith("gift_"):
            # Legacy gift link: gift_<subject>_<days>
            parts = param.split("_")
            if len(parts) == 3:
                try:
    ...
    # First-launch gift: +3 days on all subjects
    if is_new:
        for subj in TASKS.keys():
            db.set_subject_premium(user_id, subj, 3)
        bonus_messages.append("🎁 Подарок: +3 дня доступа ко всем предметам!")
